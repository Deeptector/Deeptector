...
PyObject *action_classification;
PyObject *python_close;
PyObject *pyDict;
PyObject *pyFun;

void python_embedding(char *pyFile)
{
// python embedding
    cout << "embedding start\n";
    char pyFileName[100];
    strcpy(pyFileName, pyFile);
/*
    // See [5]
    ...
    python_init = PyObject_GetAttrString(pyDict, (char*)"python_init");
    action_classification = PyObject_GetAttrString(pyDict, (char*)"action_classification");
    python_close = PyObject_GetAttrString(pyDict, (char*)"python_close");
    PyObject *arg = NULL;
    PyObject_CallMethod(python_init, arg);
    PyObject_CallMethod(action_classification, arg);
    PyObject_CallMethod(python_close, arg);

    cout << "python embedding success\n";

}

void prediction() {
PyObject_CallMethod(pModuleName, "python_init", NULL);	
	PyObject_CallMethod(pModuleName, "action_classification", NULL);
}
						...
						if(shape[0] == 1) {
							...
							if(vecs.size() == 17) {
								...
								for(int i=0; i<vecs.size() - 1; i++) {
									for(int j=0; j<54; j++) {
										if(j % 3 == 2) {
											writeFile << (vecs.at(i).at(j) + vecs.at(i+1).at(j)) / 2.0 << "," ;
										} 
										else {
											if(vecs.at(i).at(j) == 0 || vecs.at(i+1).at(j) == 0)
												writeFile << 0 << "," ;
											else 
												writeFile << (vecs.at(i).at(j) - vecs.at(i+1).at(j)) * 10 << "," ;
										}
									}
									cout << "vecs : " << vecs.size() << "     vec(" << i << "): " << vecs.at(i).size() << endl;
								}
								writeFile << endl;	
